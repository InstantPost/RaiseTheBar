  ...
  fetch(process.env.BACKEND_URI + "doctor/")
    ...
    .then(objs => {
      ...
      for (; i < num_objs; i++) {
        ...
        row.innerHTML += `
        <td>${obj.data.name}</td>
        <td><span data="doctor" data-filter-by="email" class="link filter_table_data">${
          obj.data.email
        }</span></td>
        <td><span data="doctor" data-filter-by="phone" class="link filter_table_data">${
          obj.data.phone
        }</span></td>
        <td><span data="doctor" data-filter-by="city" class="link filter_table_data">${
          obj.data.city
        }</span></td>
        <td><span data="doctor" data-filter-by="city_pin" class="link filter_table_data">${
          obj.data.city_pin
        }</span></td>
        ...
        id("doctors_data").appendChild(row);
      }
    })
    .catch(error => {
      console.log(error);
